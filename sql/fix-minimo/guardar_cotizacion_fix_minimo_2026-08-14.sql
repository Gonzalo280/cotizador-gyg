-- fix-minimo (Opción B) — mínimo comercial sobre la LÍNEA completa (impresión+terminaciones)×cantidad.
-- Aplicado directo a producción el 2026-08-14 vía apply_migration (protocolo de motor vivo:
-- PRE con rollback_guardar_cotizacion_ANTES.sql, texto revisado y aprobado por el dueño,
-- POST verificado en transacción autorrevertida — ver hilo de la sesión).
-- Ver sql/fix-minimo/rollback_guardar_cotizacion_ANTES.sql para revertir al comportamiento previo
-- (mínimo aplicado al UNITARIO, antes de multiplicar por cantidad).
--
-- Cambios respecto al RPC anterior (único bloque tocado, todo lo demás intacto:
-- terminaciones, redondeo, costo, canal, lista, inserts):
--   - v_linea numeric; agregada al declare, junto a v_minimo.
--   - Eliminado el bloque que aplicaba el mínimo sobre v_unit_p (unitario) en el tramo m².
--   - v_subtotal ahora acumula v_linea: round(v_unit_p,0)*v_cant, con el mínimo aplicado
--     recién ahí (sobre la línea completa, ya con terminaciones y cantidad incluidas).
--   - El snapshot del ítem ('subtotal' en jsonb_build_object) usa v_linea en vez de
--     v_unit_p * v_cant crudo, para que el ítem guardado refleje el mismo mínimo.
--
-- Verificado en producción (transacción con RAISE EXCEPTION al final → rollback automático,
-- sin residuo en la base):
--   P1: producto 56, 0.5×1.0 m, cantidad 10 → subtotal 27.500 (antes 50.000)
--   P2: mismo producto, cantidad 1          → subtotal 5.000
--   P5: producto 5 (metodo 'unidad'), cantidad 3 → subtotal 120.000 (sin regresión)
...
declare
  ...
  v_lam        numeric;
  v_minimo     numeric;                                  -- V4
  v_linea      numeric;                                  -- Fix-minimo: mínimo sobre línea completa (Opción B)
  v_detalle    text;                                     -- V4
  v_descr      text;                                     -- V4
...
begin
  ...
  for v_item in select * from jsonb_array_elements(p->'items') loop
    ...
    v_linea  := round(v_unit_p, 0) * v_cant;
    if v_prod.metodo = 'm2' and v_minimo > 0 and v_linea < v_minimo then
      v_linea := v_minimo;
    end if;
